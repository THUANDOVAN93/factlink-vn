<?php
	...
	$t_filetype_en = imagetype2($_FILES['t_image_en']['name']);
	$t_width_en = "180"; 
	...
	$t_filetype_jp = imagetype2($_FILES['t_image_jp']['name']);
	$t_width_jp = "180"; 
	...
	$t_filetype_vn = imagetype2($_FILES['t_image_vn']['name']);
	$t_width_vn = "180"; 
	...
	} else { $startdate = ""; $bacstatus = "d"; $bacsort = "0"; }
	...
	if ($h_bactype == 'bsc') { 
		...
		$t_width_en = "180"; $t_height_en = "86"; 
		$t_width_jp = "180"; $t_height_jp = "86";
		$t_width_vn = "180"; $t_height_vn = "86";
		$bacsort = "0";
	
	}
	
	$sql1 = "insert into flc_banner_cate (mem_id, cat_id, bac_name, bac_type, bac_page, bac_side, bac_package, bac_startdate, bac_enddate, bac_preenddate, bac_date, bac_sort, bac_status) 
					values ('$t_member', '$t_cat', '$t_name', '$h_bactype', '$t_pospage', '$t_posside', '$t_package', '$startdate', '$enddate', '$preenddate', '$nowdate', '$bacsort', '$bacstatus');";
	$result1 = mysql_query($sql1);
	
	$sql2 = "select * from flc_banner_cate order by bac_id desc limit 0,1;"; 
	$result2 = mysql_query($sql2);
	//$t_filetype=explode(".",$_FILES['t_image_vn']['name']);
	 $t_filetype=imagetype2($_FILES['t_image_en']['name']);
	 
	while ($dbarr2 = mysql_fetch_array($result2)) { $bacid = $dbarr2['bac_id']; }
		
	if ($_FILES['t_image_en']['size'] > 0) { 
		$newfile = "C".$bacid."_en.".$t_filetype;
		$imgpath = "images/banner/".$newfile;
		move_uploaded_file($_FILES['t_image_en']['tmp_name'], $imgpath); 
		$sql4 = "update flc_banner_cate set bac_filetype_en = '$t_filetype', bac_width_en = '$t_width_en', bac_height_en = '$t_height_en', bac_link_en = '$t_link_en' where bac_id = '$bacid';";
		$result4 = mysql_query($sql4);
	} 
	
	if ($_FILES['t_image_jp']['size'] > 0) { 
		$newfile = "C".$bacid."_jp.".$t_filetype;
		$imgpath = "images/banner/".$newfile;
		move_uploaded_file($_FILES['t_image_jp']['tmp_name'], $imgpath); 
		$sql4 = "update flc_banner_cate set bac_filetype_jp = '$t_filetype', bac_width_jp = '$t_width_jp', bac_height_jp = '$t_height_jp', bac_link_jp = '$t_link_jp' where bac_id = '$bacid';";
		$result4 = mysql_query($sql4);
	} 
	
	if ($_FILES['t_image_vn']['size'] > 0) { 
		$newfile = "C".$bacid."_vn.".$t_filetype;
		$imgpath = "images/banner/".$newfile;
		move_uploaded_file($_FILES['t_image_vn']['tmp_name'], $imgpath); 
		$sql4 = "update flc_banner_cate set bac_filetype_vn = '$t_filetype', bac_width_vn = '$t_width_vn', bac_height_vn = '$t_height_vn', bac_link_vn = '$t_link_vn' where bac_id = '$bacid';";
		$result4 = mysql_query($sql4);
	} 
	
	echo "<meta http-equiv = \"refresh\" content = \"0;URL = adm_bancat2.php?type=$h_bactype&start=0\">"; 
	exit();
?>