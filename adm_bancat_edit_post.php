<?php
	...
	while ($dbarr6 = mysql_fetch_array($result6)) {
		$bacsort = $dbarr6['bac_sort'];
		$bacstatus = $dbarr6['bac_status'];
	}
	...
	$sql1 = "update flc_banner_cate set mem_id = '$t_member', cat_id = '$t_cat', bac_name = '$t_name', bac_page = '$t_pospage', bac_side = '$t_posside', bac_link_en = '$t_link_en', bac_link_jp = '$t_link_jp', bac_link_vn = '$t_link_vn', bac_package = '$t_package', bac_startdate = '$startdate', bac_enddate = '$enddate', bac_preenddate = '$preenddate', bac_sort = '$bacsort', bac_status = '$bacstatus' where bac_id = '$h_bacid';";
	$result1 = mysql_query($sql1);

	if ($_FILES['t_image_en']['size'] > 0) {
		
		$newfile = "C".$h_bacid."_en.".$t_filetype_en;
	...
	if ($_FILES['t_image_jp']['size'] > 0) {
		...
		$newfile = "C".$h_bacid."_jp.".$t_filetype_jp;
		$imgpath = "images/banner/".$newfile;
	...
	if ($_FILES['t_image_vn']['size'] > 0) {
		...
		$newfile = "C".$h_bacid."_vn.".$t_filetype_vn;
		$imgpath = "images/banner/".$newfile;
