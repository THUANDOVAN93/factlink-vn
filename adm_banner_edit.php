<?php
	...
	foreach ($langCodeAllowed as $key => $langValue) {
		...
		${"banpath_$langValue"} = "images/banner/".$banid."_".$langValue.".".${"banfiletype_$langValue"};

		if (${"banfiletype_$langValue"} == 'swf') {
			${"banimagepreview_$langValue"} = "<object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0\" width=\"".${"banwidth_$langValue"}."\" height=\"".${"banheight_$langValue"}."\">
			<param name=\"movie\" value=\"".${"banpath_$langValue"}."\" />
			<param name=\"quality\" value=\"high\" />
			<embed src=\"".${"banpath_$langValue"}."\" quality=\"high\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" type=\"application/x-shockwave-flash\" width=\"".${"banwidth_$langValue"}."\" height=\"".${"banheight_$langValue"}."\"></embed>
			</object>";
		} elseif (${"banfiletype_$langValue"} == 'mp4') {

			${"banimagepreview_$langValue"} = "<a href=\"".${"banlink_$langValue"}."\" target=\"_blank\"><video width=\"".${"banwidth_$langValue"}."\" height=\"".${"banheight_$langValue"}."\" autoplay muted loop>
			<source src=\"".${"banpath_$langValue"}."\" type=\"video/mp4\">
			</video></a>";
		} else {

			${"banimagepreview_$langValue"} = "<img src=\"".${"banpath_$langValue"}."\" width=\"".${"banwidth_$langValue"}."\" border=\"0\"/>";
		}
	...
	if ($_COOKIE['vlang'] == 'en') {
		...
		$sql3 = "select * from flc_pospage order by psp_name_en asc;";
	} elseif ($_COOKIE['vlang'] == 'vn') {

		$sql3 = "select * from flc_pospage order by psp_name_vn asc;";
	} else {

		$sql3 = "select * from flc_pospage order by psp_name_jp asc;";
	}
	...
	while ($dbarr3 = mysql_fetch_array($result3)) {
		...
		if ($_COOKIE['vlang'] == 'en') {
			...
			$pspname = $dbarr3['psp_name_en'];
		} elseif ($_COOKIE['vlang'] == 'vn') {

			$pspname = $dbarr3['psp_name_vn'];
		} else {

			$pspname = $dbarr3['psp_name_jp'];
		}
		$pspcode = $dbarr3['psp_code'];

		if ($banpage == $pspcode) {

			$pspselected = "selected";
			$pspdefault = "";
		} else {
