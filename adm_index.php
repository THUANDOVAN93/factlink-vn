<?php 
	...
	// Member
	if ($_COOKIE['vlang'] == 'en') { $sql1 = "select * from flc_member where mem_expirewarning = 't' order by mem_comname_en asc;"; }
	else if ($_COOKIE['vlang'] == 'vn') { $sql1 = "select * from flc_member where mem_expirewarning = 't' order by mem_comname_vn asc;"; }
	else { $sql1 = "select * from flc_member where mem_expirewarning = 't' order by mem_comname_jp asc;"; }
	...
	while ($dbarr2 = mysql_fetch_array($result2)) {
		...
		while ($dbarr3 = mysql_fetch_array($result3)) { 
			...
			if ($_COOKIE['vlang'] == 'en') { $banpage = $dbarr3['psp_name_en']; }
			else if ($_COOKIE['vlang'] == 'vn') { $banpage = $dbarr3['psp_name_vn']; }
			else { $banpage = $dbarr3['psp_name_jp']; }
	...
	while ($dbarr4 = mysql_fetch_array($result4)) {
		...
		while ($dbarr5 = mysql_fetch_array($result5)) { 
			...
			if ($_COOKIE['vlang'] == 'en') { $banpage = $dbarr5['psp_name_en']; }
			else if ($_COOKIE['vlang'] == 'vn') { $banpage = $dbarr5['psp_name_vn']; }
			else { $banpage = $dbarr5['psp_name_jp']; }
	...
	while ($dbarr8 = mysql_fetch_array($result8)) {
		...
		while ($dbarr9 = mysql_fetch_array($result9)) { 
			...
			if ($_COOKIE['vlang'] == 'en') { $catname = $dbarr9['cat_name_en']; }
			else if ($_COOKIE['vlang'] == 'vn') { $catname = $dbarr9['cat_name_vn']; }
			else { $catname = $dbarr9['cat_name_jp']; }
		
		}//
		
		if ($bacstatus != 'd') { $bacstatus = "<a href=\"adm_bacner_set_disable.php?id=".$bacid."&type=".$type."\"><img src=\"images/icon_enable_01.png\" alt=\"".$lb_alt_on."\" width=\"20\" height=\"20\" border=\"0\" /></a>"; }
		else { $bacstatus = "<a href=\"adm_bacner_set_enable.php?id=".$bacid."&type=".$type."\"><img src=\"images/icon_disable_01.png\" alt=\"".$lb_alt_off."\" width=\"20\" height=\"20\" border=\"0\" /></a>"; }
	...
	while ($dbarr6 = mysql_fetch_array($result6)) {
		...
		while ($dbarr7 = mysql_fetch_array($result7)) { 
			...
			if ($_COOKIE['vlang'] == 'en') { $memcomname = $dbarr7['mem_comname_en']; }
			else if ($_COOKIE['vlang'] == 'vn') { $memcomname = $dbarr7['mem_comname_vn']; }
			else { $memcomname = $dbarr7['mem_comname_jp']; }
