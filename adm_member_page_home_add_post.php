<?php
	...
	$t_linetitle3 = $_POST['t_linetitle3'];
	$t_linedetail3 = $_POST['t_linedetail3'];
	...
	# region 002 use "$h_memid" to get "$memforder" from DB `flc_member`
	...
	# region 003 use "$h_memid" to get "$pagid" from DB `flc_page`
	...
	# region 004 if "$pagin" is not empty then goto "edt_page.php"
	...
	if ($h_langcode == 'en') {
		...
		/* Submit POST data into `flc_page` */
		$sql1 = "
			INSERT INTO `flc_page` (
				pag_show_en,
				pag_name_en,
				pag_name_jp,
				pag_name_vn,
		...
		$result1 = mysql_query($sql1) or die("Query insert `flc_page` failed");

		/* Get most latest COLUMN(`pag_id`) form the data we just insert into TABLE(`flc_page`) */
		...
		/* Use $pagid with POST data to insert data into `flc_home`  */
