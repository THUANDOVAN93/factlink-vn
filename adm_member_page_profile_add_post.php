<?php
	...
	/* ESCAPE ALL POST VALUE!!!! */
	$_POST = array_map('mysql_real_escape_string',$_POST);
	// echo "<pre>".print_r($_POST,true)."</pre>"; exit('STOP');
	...
	$t_comname_aka = addslashes($_POST['t_comname_aka']);
	...
	$t_seocom = addslashes($_POST['t_seocom']);
	$t_seokey = addslashes($_POST['t_seokey']);
	...
	$sql5 = "select * from flc_page where mem_id = '$h_memid' and pag_type = 'prf';"; 
	$result5 = mysql_query($sql5);
	while ($dbarr5 = mysql_fetch_array($result5)) { $pagid = $dbarr5['pag_id']; }
	
	if ($pagid != '') { echo "<meta http-equiv = \"refresh\" content = \"0;URL = adm_member_page.php?id=$memid\">"; exit(); }
	
	if ($t_estday != '' && $t_estmonth != '' && $t_estyear != '') { $t_establish = $t_estday." ".$t_estmonth." ".$t_estyear; } else { $t_establish = ""; }
	
	$sql6 = "select * from flc_member where mem_id = '$h_memid';"; 
	$result6 = mysql_query($sql5);
	while ($dbarr6 = mysql_fetch_array($result6)) { 
		$memcomnameen = $dbarr6['mem_comname_en']; 
		$memcomnamejp = $dbarr6['mem_comname_jp']; 
		$memcomnamevn = $dbarr6['mem_comname_vn']; 
	}
	
	if ($h_langcode == 'en') {
		
		$sql1 = "insert into flc_page (pag_show_en, pag_name_en, pag_name_jp, pag_name_vn, pag_pagetitle_en, pag_title_color, pag_profile_color, pag_title_en, pag_detail_en, pag_sort, pag_type, mem_id)";
		
		$sql2 = "update flc_member set mem_comname_en = '$t_comname', mem_comname_aka = '$t_comname_aka', mem_pername_en = '$t_pername', mem_posname_en = '$t_posname', 
						...
						mem_product_en = '$t_product', mem_seocomdesc = '$t_seocom', mem_seokeyword = '$t_seokey', 
		...
		$sql1 = "insert into flc_page (pag_show_vn, pag_name_en, pag_name_jp, pag_name_vn, pag_pagetitle_vn, pag_title_color, pag_profile_color, pag_title_vn, pag_detail_vn, pag_sort, pag_type, mem_id)";
		
		$sql2 = "update flc_member set mem_comname_vn = '$t_comname', mem_comname_aka = '$t_comname_aka', mem_pername_vn = '$t_pername', mem_posname_vn = '$t_posname', mem_pername_en = '$t_pername_en', mem_posname_en = '$t_posname_en', 
						...
						mem_product_vn = '$t_product', mem_seocomdesc = '$t_seocom', mem_seokeyword = '$t_seokey', 
		...
		$sql1 = "insert into flc_page (pag_show_jp, pag_name_en, pag_name_jp, pag_name_vn, pag_pagetitle_jp, pag_title_color, pag_profile_color, pag_title_jp, pag_detail_jp, pag_sort, pag_type, mem_id)";
		
		$sql2 = "update flc_member set mem_comname_jp = '$t_comname', mem_comname_aka = '$t_comname_aka', mem_pername_jp = '$t_pername', mem_posname_jp = '$t_posname', mem_pername_en = '$t_pername_en', mem_posname_en = '$t_posname_en', 
						...
						mem_product_jp = '$t_product', mem_seocomdesc = '$t_seocom', mem_seokeyword = '$t_seokey', 
	...
	$sql1 = $sql1." values ('$t_setlang', '$t_pagname', '$t_pagname', '$t_pagname', '$t_pagpagetitle', '$t_clf', '$t_clfdetail', '$t_pagtitle', '$t_pagdetail', '$t_pagsort', 'prf', '$h_memid');";
	$result1 = mysql_query($sql1);
	
	$sql4 = "select * from flc_page order by pag_id desc limit 0,1;"; 
	...
	if ($_FILES['t_image']['size'] > 0) { 
		...
		$sql3 = "update flc_page set pag_image = 't', pag_image_width = '$t_imagewidth', pag_image_link = '$t_imagelink', pag_image_side = '$t_imageside' where pag_id = '$pagid';";
	...
	$fileindex = fopen("$dirpath/index.php", "x");
	
	$old_umask = umask(0);
	chmod("$dirpath/index.php", 0777);
	umask($old_umask);
	...
	\$sql0 = \"select * from flc_page where pag_type = 'prf' and mem_id = '$h_memid';\"; 
	\$result0 = mysql_query(\$sql0);
	...
	echo \"<meta http-equiv = \\\"refresh\\\" content = \\\"0;URL = ../../mem_profile.php?id=$h_memid&page=$pagid&lang=\$langset\\\">\";
	exit();
