<?php 
	...
	if ($mempackage == '') { echo "<meta http-equiv = \"refresh\" content = \"0;URL = adm_member_searchcate.php?start=0\">"; exit(); }
	
	$sql2 = "select * from flc_category where cat_id = '$memcate';"; 
	$result2 = mysql_query($sql2);
	while ($dbarr2 = mysql_fetch_array($result2)) {
		
		if ($_COOKIE['vlang'] == 'en') { $catname = $dbarr2['cat_name_en']; }
		else if ($_COOKIE['vlang'] == 'vn') { $catname = $dbarr2['cat_name_vn']; }
		else { $catname = $dbarr2['cat_name_jp']; }
