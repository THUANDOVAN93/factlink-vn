<?php 
	...
	while ($dbarr3 = mysql_fetch_array($result3)) {
		...
		if ($_COOKIE['vlang'] == 'en') { $nwstitle = $dbarr3['nws_title_en']; $nwssum = $dbarr3['nws_compend_en']; $nwsdetail = $dbarr3['nws_detail_en']; }
		else if ($_COOKIE['vlang'] == 'vn') { $nwstitle = $dbarr3['nws_title_vn']; $nwssum = $dbarr3['nws_compend_vn']; $nwsdetail = $dbarr3['nws_detail_vn']; }
		else { $nwstitle = $dbarr3['nws_title_jp']; $nwssum = $dbarr3['nws_compend_jp']; $nwsdetail = $dbarr3['nws_detail_jp']; }
		
		// for jp only
		$nwstitle = $dbarr3['nws_title_jp']; $nwssum = $dbarr3['nws_compend_jp']; $nwsdetail = $dbarr3['nws_detail_jp'];		
	...
	$tpl->assign("##nwssum##", stripslashes(html($nwssum)));
	$tpl->assign("##nwsdetail##", stripslashes(html($nwsdetail)));
