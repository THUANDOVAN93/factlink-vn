<?php 
	...
	while ($dbarr3 = mysql_fetch_array($result3)) {
		...
		$nwsshow = $dbarr3['nws_show']; if ($nwsshow == 't') { $nwsshow = "checked"; } else { $nwsshow = ""; }
		$nwsstatus = $dbarr3['nws_status']; if ($nwsstatus == 'd') { $nwsstatus = "checked"; } else { $nwsstatus = ""; }
	...
	if ($_COOKIE['vlang'] == 'en') { $sql1 = "select * from flc_news_genre order by nwg_name_en asc;"; }
	else if ($_COOKIE['vlang'] == 'vn') { $sql1 = "select * from flc_news_genre order by nwg_name_vn asc;"; }
	else { $sql1 = "select * from flc_news_genre order by nwg_name_jp asc;"; }
	...
	while ($dbarr1 = mysql_fetch_array($result1)) {
		...
		$nwgid = $dbarr1['nwg_id'];
		if ($nwgid == $nwsnwgid) { $nwgselected = "selected"; $nwgdefault = ""; } else { $nwgselected = ""; $nwgdefault = "selected"; }
	...
	if ($_COOKIE['vlang'] == 'en') { $sql2 = "select * from flc_news_editor order by nwe_name_en asc;"; }
	else if ($_COOKIE['vlang'] == 'vn') { $sql2 = "select * from flc_news_editor order by nwe_name_vn asc;"; }
	else { $sql2 = "select * from flc_news_editor order by nwe_name_jp asc;"; }
	$result2 = mysql_query($sql2);
	while ($dbarr2 = mysql_fetch_array($result2)) {
		
		if ($_COOKIE['vlang'] == 'en') { $nwename = $dbarr2['nwe_name_en']; }
		else if ($_COOKIE['vlang'] == 'vn') { $nwename = $dbarr2['nwe_name_vn']; }
		else { $nwename = $dbarr2['nwe_name_jp']; }
		$nweid = $dbarr2['nwe_id'];
		if ($nweid == $nwsnweid) { $nweselected = "selected"; $nwedefault = ""; } else { $nweselected = ""; $nwedefault = "selected"; }
