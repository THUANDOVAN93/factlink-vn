...
//
// RCS Revision
//   @(#) $Id: class.rFastTemplate.php,v 1.19 2001/10/03 21:24:01 roland Exp $
...
//
//    define_dynamic
//       Dynamic templates are handled the same as "normal" templates.
//    clear_dynamic
//       This was to undo the effects of define_dynamic which no longer
//       exists.  See clear().
//    clear_parse
//       Same as clear.
//    clear_all
...
class rFastTemplate {
   ...
   function &load ($file) {
	   ...
	   if(!array_key_exists('load',$this->DEBUG)) {
		   $this->DEBUG['load'] = null;
	   }
   ...
   function &parse_internal_1 ($tag, $rest = '') {
	  ...
	  if(!array_key_exists('parse_internal_1',$this->DEBUG)) {
		  $this->DEBUG['parse_internal_1'] = null;
	  }
         ...
         if (preg_match ($this->REGEX_DYNBEG, $rest, $dynbeg)) {
            ...
            while (preg_match ($this->REGEX_DYNEND, $rest, $dynend)) {
               ...
               if ($dynbeg[2] == $dynend[2]) {
				  ...
				  /* Fix missing index 'clear' */
				  if(!array_key_exists($dynend[2],$this->TEMPLATE)) {
					  $this->TEMPLATE[$dynend[2]] = array();
				  }
				  
				  if(!array_key_exists('clear',$this->TEMPLATE[$dynend[2]])) {
					  $this->TEMPLATE[$dynend[2]]['clear'] = null;
				  }
				  if(!array_key_exists('loaded',$this->TEMPLATE[$dynend[2]])) {
					  $this->TEMPLATE[$dynend[2]]['loaded'] = null;
				  }
   ...
   function parse_internal ($tag) {
	   ...
	   if(!array_key_exists('parse_internal',$this->DEBUG)) {
		   $this->DEBUG['parse_internal'] = null;
	   }
      ...
      if (gettype($tag) == 'array') {
         ...
         if ($this->DYNAMIC) {
			...
			if(!array_key_exists('parsed',$this->TEMPLATE[$tag])) {
				$this->TEMPLATE[$tag]['parsed'] = array();
			}
   ...
   function subst ($handle, $tag, $autoload = true) {
	  ...
	  if(empty($this->DEBUG['subst'])) {
		$this->DEBUG['subst'] = null;
	  }
...
//                   $key = '{'.$key.'}';  // Disabled by Tuna
      ...
      if ($append) {
		 ...
		 if(!array_key_exists('result',$this->TEMPLATE[$tag])) {
			$this->TEMPLATE[$tag]['result'] = null;
		 }
